...
def mark_done(rid):
    # Accept explicit patient_id — needed when doctor views another patient's reminders
    data = request.get_json(silent=True) or {}
    pid = data.get("patient_id") or _get_patient_id()
...
def upload_scan_for_reminder(rid):
    # patient_id passed as form field (multipart request)
    pid = request.form.get("patient_id") or _get_patient_id()
